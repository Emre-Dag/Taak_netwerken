...
int connection( int internet_socket )
{
	...
	ip_client=get_ip_address( &client_internet_address );
...
char* get_ip_address( struct sockaddr_storage * ip )
{
	...
	char* ip_string_p=ip_string;
	
	if( ip->ss_family == AF_INET )
	{ // IPv4
		struct sockaddr_in *ipv4 = (struct sockaddr_in *)ip;
		ip_address = &(ipv4->sin_addr);
		inet_ntop( ip->ss_family, ip_address, ip_string, INET_ADDRSTRLEN );
	}
	else
	{ // IPv6
		struct sockaddr_in6 *ipv6 = (struct sockaddr_in6 *)ip;
		ip_address = &(ipv6->sin6_addr);
		inet_ntop( ip->ss_family, ip_address, ip_string, INET6_ADDRSTRLEN );
	}
