...
int initialization( struct sockaddr ** internet_address, socklen_t * internet_address_length )
{
	...
	int getaddrinfo_return = getaddrinfo( "192.168.0.118", "24042", &internet_address_setup, &internet_address_result );
...
void execution( int internet_socket, struct sockaddr * internet_address, socklen_t internet_address_length )
{
	while(1)
	{
		...
		printf("Hoeveel pakketten wilt u ontvangen?\n(Schrijf \"shutdown\" voor een shutdown)\nSend: ");
		gets(str1);
		printf("\n");
		
		number_of_bytes_send = sendto( internet_socket, str1, strlen(str1), 0, internet_address, internet_address_length );
		if( number_of_bytes_send == -1 )
		{
			perror( "sendto" );
		}
		if( strcmp (str1 , "shutdown")==0 )
		{
			break;
		}
		...
		printf("Gewenste aantal:%d\tAangekomen aantal:%d\tPercentage%d%%\n",	atoi(str1),	teller,	(teller/atoi(str1))	* 100);
	}
}
